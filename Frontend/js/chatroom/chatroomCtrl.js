angular.module('messangerApp')
	...
	.controller('chatroomCtrl', function ($scope, chatroomService, $location, $anchorScroll) {
		...
		$scope.sendNewMessage = function (i) {
			...
			var chatbox = $('#message-container');
			var height = chatbox[0].scrollHeight - chatbox.height();
			chatbox.scrollTop(height + 200);
			// $('#message-container').scrollTop($('#message-container')[0].scrollHeight + 200);

			// 	var chatBox = document.getElementById('message-container');
			// 	chatBox.scrollTop = 400 + 10 + ($scope.messages.length * 260);
		}
